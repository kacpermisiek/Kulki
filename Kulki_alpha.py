...
class Gracz(object):
    ...
    def rysuj(self, okno):
        ...
        if self.kopniecie is True and self.kopniecie_nowe is True:
            ...
            pygame.gfxdraw.filled_circle(okno, x, y, otoczka_promien, (0, 0, 0))
            pygame.gfxdraw.aacircle(okno, x, y, otoczka_promien, (0, 0, 0))
...
def redraw_okno():  # rysowanie elementow okna gry
    ...
    # linie
    pygame.draw.rect(okno, bramka_linia_kolor, (boisko_naroznikx - bramka_linia_grubosc // 2, boisko_narozniky -
                                                bramka_linia_grubosc // 2, bramka_linia_grubosc, boisko_wysokosc +
                                                bramka_linia_grubosc))

    pygame.draw.rect(okno, liniasrodkowa_kolor, (
        okno_szerokosc - boisko_naroznikx - bramka_linia_grubosc // 2, boisko_narozniky - bramka_linia_grubosc // 2,
        bramka_linia_grubosc, boisko_wysokosc + bramka_linia_grubosc))

    pygame.draw.rect(okno, liniasrodkowa_kolor, (
        boisko_naroznikx - bramka_linia_grubosc // 2, boisko_narozniky - bramka_linia_grubosc // 2,
        boisko_szerokosc + bramka_linia_grubosc, bramka_linia_grubosc))

    pygame.draw.rect(okno, bramka_linia_kolor, (
        boisko_naroznikx - bramka_linia_grubosc // 2, okno_wysokosc - boisko_narozniky - bramka_linia_grubosc // 2,
        boisko_szerokosc + bramka_linia_grubosc, bramka_linia_grubosc))
    ...
    pygame.gfxdraw.filled_circle(okno, pilka_start[0], pilka_start[1], liniasrodkowa_promien - liniasrodowa_grubosc,
                                 boisko_kolor)

    pygame.gfxdraw.aacircle(okno, pilka_start[0], pilka_start[1], liniasrodkowa_promien - liniasrodowa_grubosc,
                            boisko_kolor)

    pygame.draw.rect(okno, liniasrodkowa_kolor, (
        okno_szerokosc // 2 - liniasrodowa_grubosc // 2, boisko_narozniky, liniasrodowa_grubosc, boisko_wysokosc))
    ...
    # pokazywanie czasu
    if czas_format(uplynelo) > czas_gry:
        timetpl = czas_gry
    else:
...
def kolizja(obj1, obj2):
    ...
    # nieelastyczna formula kolizji
    obj1_predkosc_normalna2 = (odbicie * obj2.masa * (
            obj2_predkosc_normalna - obj1_predkosc_normalna) + obj1.masa * obj1_predkosc_normalna + obj2.masa *
                               obj2_predkosc_normalna) / (obj1.masa + obj2.masa)

    obj2_predkosc_normalna2 = (odbicie * obj1.masa * (obj1_predkosc_normalna - obj2_predkosc_normalna) +
                               obj2.masa * obj2_predkosc_normalna + obj1.masa * obj1_predkosc_normalna) / \
                              (obj2.masa + obj1.masa)
    ...
    obj1.predkosc = obj1_predkosc_normalna2 * np.array(kolizja_normal) + obj1_predkosc_styczna * np.array(
        kolizja_styczna)

    obj2.predkosc = obj2_predkosc_normalna2 * np.array(kolizja_normal) + obj2_predkosc_styczna * np.array(
        kolizja_styczna)

    obj1.poz = srodek_masy + ((obj1.promien + obj2.promien) + odbicie * (
            obj1.promien + obj2.promien - dystans)) * kolizja_normal * obj2.masa / (obj1.masa + obj2.masa)

    obj2.poz = srodek_masy - ((obj1.promien + obj2.promien) + odbicie * (
            obj1.promien + obj2.promien - dystans)) * kolizja_normal * obj1.masa / (obj1.masa + obj2.masa)
...
def zatrzymaj_pilke(pilka):
    if pilka.poz[0] <= pilka_polex[0] or pilka.poz[0] >= pilka_polex[1]:
        if bramka_y[0] <= pilka.poz[1] <= bramka_y[1]:
            pass
        else:
...
for i in range(czerwoni_ilosc):
    czerwoni.append(
        Gracz(czerwoni_start[0] + 0 * np.random.uniform(-1, 1), czerwoni_start[1] + 0 * np.random.uniform(-1, 1),
              gracz_czerwony_kolor))

for i in range(niebiescy_ilosc):
    niebiescy.append(
        Gracz(niebiescy_start[0] + 0 * np.random.uniform(-1, 1), niebiescy_start[1] + 0 * np.random.uniform(-1, 1),
              gracz_niebieski_kolor))
...
while run:
    ...
    if mozna_ruszac(czas_format(uplynelo)):
    ...
    if mozna_ruszac(czas_format(uplynelo)):
        for i in gracze:
            if i.kopniecie and i.kopniecie_nowe:
